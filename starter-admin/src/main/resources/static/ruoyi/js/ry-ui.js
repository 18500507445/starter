...
(function ($) {
    $.extend({
        ...
        modal: {
            ...
            // 弹出层指定宽度
            openOpt: function (options) {
                var _url = $.common.isEmpty(options.url) ? "/404.html" : options.url;
                var _title = $.common.isEmpty(options.title) ? "系统窗口" : options.title;
                var _width = $.common.isEmpty(options.width) ? "800" : options.width;
                var _height = $.common.isEmpty(options.height) ? ($(window).height() - 50) : options.height;
                var _check = $.common.isEmpty(options.check) ? "确认" : options.check;
                var _close = $.common.isEmpty(options.close) ? "关闭" : options.close;
                var _btn = ['<i class="fa fa-check"></i> ' + _check, '<i class="fa fa-close"></i> ' + _close];
                var callback = $.common.isEmpty(callback) ? "" : options.callback;
                if ($.common.isEmpty(callback)) {
                    callback = function (index, layero) {
                        var iframeWin = layero.find('iframe')[0];
                        iframeWin.contentWindow.submitHandler(index, layero);
